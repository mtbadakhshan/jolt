#![no_std]
#![doc = "Workspace-local fork of `ml-dsa 0.1.0-rc.11` for the Jolt zkVM. See \
          `crates/ml-dsa-jolt/Cargo.toml` for the diff from upstream."]
#![doc(
...
pub use crate::{
    algebra::{Elem, NttMatrix, NttPolynomial, NttVector, Polynomial, Vector},
    param::{
        EncodedSignature, EncodedVerifyingKey, ExpandedSigningKeyBytes, MlDsaParams, ParameterSet,
        SignatureParams, SigningKeyParams, VerifyingKeyParams,
    },
...
use crate::{algebra::AlgebraExt, crypto::H, hint::Hint, param::QMinus1};
...
impl<P: MlDsaParams> Signature<P> {
    ...
    /// Borrow the `c̃` (commitment hash) field of the signature.
    ///
    /// Useful for callers that want to compute `c = SampleInBall(c̃)`
    /// outside the proof boundary; pair with [`crate::sample_in_ball`].
    #[must_use]
...
/// Compute `tr = SHAKE256(pk_encoded, 64 B)`, the BUFF tag that binds messages
/// to a specific verifying key (FIPS 204 §5.3 step 5).
///
/// This is one of the public hashing steps that callers can lift out of a
/// proof boundary: pass the result into [`VerifyingKey::new_with_precomputed`]
/// instead of letting `VerifyingKey::decode` recompute it.
#[must_use]
pub fn compute_tr(pk_encoded: &[u8]) -> B64 {
    H::default().absorb(pk_encoded).squeeze_new()
}

/// Compute `μ = SHAKE256(tr ‖ M, 64 B)` matching `ML-DSA.Verify_internal`
/// (FIPS 204 §5.3 step 6, internal mode — no domain separator / context).
///
/// Pair with [`VerifyingKey::raw_verify_with_precomputed`] /
/// [`VerifyingKey::verify_mu`] when the signer used `sign_internal`.
/// Most callers want [`compute_mu_with_context`] instead — that's the
/// `μ` used by the public-facing `Sign` / `Verify` flows.
#[must_use]
pub fn compute_mu_internal(tr: &B64, msg: &[&[u8]]) -> B64 {
    MuBuilder::internal(tr, msg)
}

/// Compute `μ = SHAKE256(tr ‖ 0x00 ‖ len(ctx) ‖ ctx ‖ M, 64 B)` matching the
/// public-facing `ML-DSA.Sign` / `ML-DSA.Verify` flow (FIPS 204 §5.3 with
/// domain separator and context prefix).
///
/// `ctx.len()` MUST be ≤ 255 (the FIPS 204 limit); larger values are
/// truncated to a single byte and will cause verify mismatches.
#[must_use]
pub fn compute_mu_with_context(tr: &B64, ctx: &[u8], msg: &[&[u8]]) -> B64 {
    MuBuilder::new(tr, ctx).message(msg)
}
...
mod test {
    ...
    // Sanity check: `new_with_precomputed` + `raw_verify_with_precomputed`
    // accept signatures that the standard verify path accepts, when fed the
    // same precomputed inputs the host would supply (`A_hat`, `tr`, `μ`, `c`).
    // Covers both μ shapes (`compute_mu_internal` for `*_internal` signers,
    // `compute_mu_with_context` for the public-facing `Sign`/`Verify` flow).
    #[test]
    fn precomputed_verify_round_trip_internal() {
        fn check<P: MlDsaParams>() {
            ...
            let (rho, t1_enc) = P::split_vk(&pk_enc);
            let t1 = P::decode_t1(t1_enc);
            let a_hat: NttMatrix<P::K, P::L> = expand_a(rho);
            let tr = compute_tr(&pk_enc);
            let mu = compute_mu_internal(&tr, &[M]);
            let c = sample_in_ball(sig.c_tilde(), P::TAU);

            let vk_pre = VerifyingKey::<P>::new_with_precomputed(rho.clone(), t1, a_hat, tr);
            assert!(
                vk_pre.raw_verify_with_precomputed(&mu, &c, &sig),
                "precomputed (internal) verify rejected a valid signature for {}",
    ...
    #[test]
    fn precomputed_verify_round_trip_with_context() {
        fn check<P: MlDsaParams>() {
            let ssk = SigningKey::<P>::from_seed(&Array::default());
            let vk = ssk.verifying_key();
            ...
            let M = b"Hello world";
            let ctx: &[u8] = b"";
            // `Signer::sign` uses the deterministic public-facing flow,
            // which builds μ via `MuBuilder::new(tr, ctx)` (with domain
            // separator and ctx prefix).
            let sig: Signature<P> = signature::Signer::sign(&ssk, M);
            assert!(vk.verify_with_context(M, ctx, &sig));
            ...
            let a_hat: NttMatrix<P::K, P::L> = expand_a(rho);
            let tr = compute_tr(&pk_enc);
            let mu = compute_mu_with_context(&tr, ctx, &[M]);
            let c = sample_in_ball(sig.c_tilde(), P::TAU);

            let vk_pre = VerifyingKey::<P>::new_with_precomputed(rho.clone(), t1, a_hat, tr);
            assert!(
                vk_pre.raw_verify_with_precomputed(&mu, &c, &sig),
                "precomputed (with_context) verify rejected a valid signature for {}",
                core::any::type_name::<P>()
            );
        }
        check::<MlDsa44>();
