...
// --- Jolt Keccak inline dispatch -----------------------------------------
//
// On RISC-V the Keccak-f[1600] permutation is the Jolt inline opcode.
// On the host it falls back to a standard software permutation (ported
// from the upstream `keccak 0.2.0` crate, Apache-2.0 / MIT).

#[cfg(any(target_arch = "riscv32", target_arch = "riscv64"))]
fn keccak_f1600(state: &mut [u64; 25]) {
    use jolt_inlines_keccak256::{INLINE_OPCODE, KECCAK256_FUNCT3, KECCAK256_FUNCT7};
    // SAFETY: `state` is a `[u64; 25]`, naturally u64-aligned. The Jolt
    // Keccak inline reads the pointer from `rs1` and permutes the 200 bytes
    // in place; no other memory is touched.
    unsafe {
        core::arch::asm!(
...
fn keccak_f1600(state: &mut [u64; 25]) {
    soft_keccak::keccak_f1600(state);
}

#[cfg(not(any(target_arch = "riscv32", target_arch = "riscv64")))]
mod soft_keccak {
    //! Standard Keccak-f[1600], ported from `keccak 0.2.0` (Apache-2.0 / MIT).
...
impl KeccakState {
    ...
    fn process(&mut self) {
        keccak_f1600(&mut self.0);
    }

    // The original hand-rolled Keccak-f[1600] is replaced by
    // `keccak_f1600()` defined at module level, which dispatches to
    // the Jolt Keccak inline on RISC-V and a standard software
    // permutation on the host.
    //
    // The original used an alternate internal representation (some
    // words inverted at entry/exit for a chi-step optimization), but
    // the standard representation is what `inject`/`flip`/`extract`
    // use, so a standard Keccak-f[1600] is a drop-in replacement.
    #[cfg(any())] // dead code — kept for reference
    fn _original_process(&mut self) {
