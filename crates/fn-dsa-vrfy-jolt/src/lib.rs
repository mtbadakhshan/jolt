...
pub use fn_dsa_comm::hash_to_point;
pub use fn_dsa_comm::{
...
/// Compute `hashed_key = SHAKE256(pk, 64 B)`, the BUFF tag that FN-DSA
/// uses to bind the message hash to a specific verifying key.
///
/// This is one of the public hashing steps that callers can lift out of a
/// proof boundary: pass the result into [`VerifyingKey512::from_parts`]
/// instead of letting [`VerifyingKey512::decode`] recompute it.
pub fn compute_hashed_key(pk: &[u8]) -> [u8; 64] {
...
macro_rules! vrfy_key_impl {
    ($typename:ident, $logn_min:expr, $logn_max:expr) => {
        ...
        impl $typename {
            /// Construct a verifying key from already-decoded parts.
            ///
            /// `h_ntt` must be the polynomial in NTT form (as produced by
            /// `decode_inner`, i.e. after `modq_decode` + `ext_to_int` +
            /// `int_to_NTT`). `hashed_key` must be `SHAKE256(pk, 64)`.
            ///
            /// Use this when those values have been computed on the host
            /// to avoid running SHAKE256 and NTT inside the proof.
            pub fn from_parts(logn: u32, h_ntt: &[u16], hashed_key: [u8; 64]) -> Option<Self> {
            ...
            /// Verify a signature using a precomputed `c = hash_to_point(...)`.
            ///
            /// `c` must be 512 (or 1024) `u16` coefficients in external form,
            /// exactly as `hash_to_point` would produce. The caller is
            /// responsible for computing `c` correctly from `(nonce, hashed_key,
            /// ctx, id, hv)` — see [`fn_dsa_comm::hash_to_point`].
            pub fn verify_with_precomputed_c(&self, sig: &[u8], c: &[u16]) -> bool {
                let logn = self.logn;
                let n = 1usize << logn;
                let mut tmp_i16 = [0i16; 1 << ($logn_max)];
                let mut tmp_u16 = [0u16; 2 << ($logn_max)];
                verify_inner_with_precomputed_c(
                    logn,
                    &self.h[..n],
                    sig,
                    c,
            ...
            /// Borrow the `hashed_key` (BUFF tag) stored in this key.
            pub fn hashed_key(&self) -> &[u8; 64] {
                &self.hashed_key
            }

            /// Borrow the `h` polynomial (NTT form) stored in this key.
            pub fn h_ntt(&self) -> &[u16] {
                let n = 1usize << self.logn;
                &self.h[..n]
            }

            /// The `logn` parameter (degree = 2^logn).
            pub fn get_logn(&self) -> u32 {
                self.logn
            }
        }
...
/// Like `verify_inner` but takes a precomputed `c` polynomial (in external
/// form, as `hash_to_point` would produce) instead of calling
/// `hash_to_point` itself. The `sig` is still needed for the nonce (but its
/// nonce field is not read — only the header, Comp-encoded s2, and sizes).
fn verify_inner_with_precomputed_c(
    logn: u32,
    h: &[u16],
    sig: &[u8],
    c_ext: &[u16],
    ...
    // t1 <- c (precomputed, in external form → convert to internal)
    t1[..n].copy_from_slice(&c_ext[..n]);
    mq::mqpoly_ext_to_int(logn, t1);
